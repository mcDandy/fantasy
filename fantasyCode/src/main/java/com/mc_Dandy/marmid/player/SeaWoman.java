		...
		if (player.isInWater()) {
			...
			if (compound.getInteger("level") > 0) // is marmid?
			{
				...
				if (tick == (20 * 5)) {
					...
					// false);
					// Potions(player, Potion.waterBreathing, 20/*sec*/, 0,
					// true, false);
				}
			}
		}
		/*
		 * @SubscribeEvent(priority = EventPriority.NORMAL) void
		 * level_up(ItemTossEvent e){compound.setInteger("level",
		 * compound.getInteger("level")+1);}
		 */}
